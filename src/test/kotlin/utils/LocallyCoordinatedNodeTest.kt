...
class LocallyCoordinatedNodeTest {
    ...
    @org.junit.Test
    fun rotationDegree() {
        val firstCase = LocallyCoordinatedNode(0, 0,
        ...
        val secCase = LocallyCoordinatedNode(1, 1,
                State.createState(setOf("a", "b"), "a"), 1)
        ...
        assert(LocallyCoordinatedNode(1, -1,
                State.createState(setOf("a", "b"), "a"), 1) == tt90)
        assert(LocallyCoordinatedNode(-1, -1,
                State.createState(setOf("a", "b"), "a"), 1) == tt180)
        assert(LocallyCoordinatedNode(-1, 1,
                State.createState(setOf("a", "b"), "a"), 1) == tt270)
        assert(secCase == tt360)

        val thirdCase = LocallyCoordinatedNode(1, 2,
                State.createState(setOf("a", "b"), "a"), 1)
