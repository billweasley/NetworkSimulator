...
class GridNode(graph: AbstractGraph?, id: String?,
    ...
    fun getOppositeConnectionCoordinte(port: Port): Pair<Double,Double>{
        val pos = Toolkit.nodePosition(this)
        return when(port){
            Port.UP -> centeredRotate(rotation, pos[0], (pos[1] + portLength + edgeLength))
            Port.DOWN -> centeredRotate(rotation, pos[0], (pos[1] - portLength - edgeLength))
            Port.RIGHT -> centeredRotate(rotation, (pos[0] + portLength + edgeLength), pos[1])
            Port.LEFT -> centeredRotate(rotation,(pos[0] - portLength - edgeLength), pos[1])
        }
    }
