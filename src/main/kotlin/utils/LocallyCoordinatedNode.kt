...
class LocallyCoordinatedNode(x: Int,
                             y: Int,
                             state: State, index: Int) : Node(state = state, index = index) {
    var up: LocallyCoordinatedNode? = null
    var down: LocallyCoordinatedNode? = null
    var left: LocallyCoordinatedNode? = null
    var right: LocallyCoordinatedNode? = null
    var coordinate = Coordinate.of(x, y)

    companion object {
        fun canConnect(nodeA: LocallyCoordinatedNode, portA: Port, nodeB: LocallyCoordinatedNode, portB: Port): Boolean {
...
//      180
        ...
        fun canSeperata(nodeA: LocallyCoordinatedNode, portA: Port, nodeB: LocallyCoordinatedNode, portB: Port): Boolean {
            if (nodeA == nodeB) return false
        ...
        private fun populateAndRotateNodes(origin: LocallyCoordinatedNode, degree: Int): Set<LocallyCoordinatedNode> {
            return rotateAllNodes(origin, degree, populateCoordinate(origin))
        }

        private fun populateCoordinate(origin: LocallyCoordinatedNode):
                Set<LocallyCoordinatedNode> {
            val queue = LinkedList<LocallyCoordinatedNode>()
            queue.offer(origin)
            val hasVisitedIndexSet = HashSet<LocallyCoordinatedNode>()
        ...
        private fun rotateAllNodes(origin: LocallyCoordinatedNode, degree: Int,
                                   nodes: Set<LocallyCoordinatedNode>): Set<LocallyCoordinatedNode> {
        ...
        // IN-CLOCK direction rotation
        private fun rotateOneNode(target: LocallyCoordinatedNode, degree: Int, centralCoordinate: LocallyCoordinatedNode) {
            val rad = degree * Math.PI / 180
            val transferMat = mat[
                    Math.cos(rad), Math.sin(rad) end
                            0 - Math.sin(rad), Math.cos(rad)
